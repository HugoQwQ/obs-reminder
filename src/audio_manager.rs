...
impl std::fmt::Display for AudioError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AudioError::Other(s) => write!(f, "Audio error: {}", s),
        }
    }
}
...
impl AudioManager {
    ...
    pub fn add_audio_file<P: AsRef<Path>>(&self, source_path: P) -> Result<String, AudioError> {
        ...
        log::info!("Audio file cached: {} -> {}", source_path.display(), dest_path.display());
    ...
    pub fn cleanup_unused_files(&self, used_ids: &[String]) -> Result<(), AudioError> {
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.is_file() {
                if let Some(filename) = path.file_stem().and_then(|s| s.to_str()) {
                    if !used_ids.contains(&filename.to_string()) {
                        fs::remove_file(&path)?;
